...
ifneq ($(DOCA_HOME), 0)
ifeq ($(DOCA_HDR_FILE), 1)
NVCUFLAGS_VERIFIABLE := $(NVCUFLAGS)
NVCUFLAGS += -I$(DOCA_HOME)/include
CXXFLAGS += -DALLOW_EXPERIMENTAL_API
CXXFLAGS += -DDOCA_ALLOW_EXPERIMENTAL_API
# DOCA_LIB_STATIC := $(DOCA_HOME)/lib/x86_64-linux-gnu/libdoca_gpunetio_device.a
NVLDFLAGS_VERIFIABLE := $(NVLDFLAGS)
# NVLDFLAGS += $(DOCA_LIB_STATIC)
else
NVCUFLAGS_VERIFIABLE := $(NVCUFLAGS)
NVCUFLAGS += -I$(DOCA_HOME)/include -rdc=true
CXXFLAGS += -DALLOW_EXPERIMENTAL_API
CXXFLAGS += -DDOCA_ALLOW_EXPERIMENTAL_API
DOCA_LIB_STATIC := $(DOCA_HOME)/lib/x86_64-linux-gnu/libdoca_gpunetio_device.a
NVLDFLAGS_VERIFIABLE := $(NVLDFLAGS)
